...
const baseURL = process.env.REACT_APP_CORE_API + '/transactions/accessrequests/logs';
const currentUser = getCurrentUser();

export const accessrequestLogsAPI = createDynamicResourceAPI({
  resource: "AccessRequestLogs",
  tagTypes: ["AccessRequestLogs"],
      ...
      [`export${resource}`]: builder.mutation({
        ...
        async onQueryStarted(params, { queryFulfilled }) {
          try {
            ...
            link.href = url;
            link.download = `AccessRequestLogs-${new Date().toISOString().split("T")[0]}.xlsx`;
...
export const {
  useGetAccessRequestLogsQuery,
  useExportAccessRequestLogsMutation
} = accessrequestLogsAPI;
