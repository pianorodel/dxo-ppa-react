...
const baseURL = process.env.REACT_APP_FMS_API + '/transactions/obligations/logs';
const currentUser = getCurrentUser();

export const obligationLogsAPI = createDynamicResourceAPI({
  resource: "ObligationLogs",
  tagTypes: ["ObligationLogs"],
      ...
      [`export${resource}`]: builder.mutation({
        ...
        async onQueryStarted(params, { queryFulfilled }) {
          try {
            ...
            link.href = url;
            link.download = `ObligationLogs-${new Date().toISOString().split("T")[0]}.xlsx`;
...
export const {
  useGetObligationLogsQuery,
  useExportObligationLogsMutation
} = obligationLogsAPI;
