...
const baseURL = process.env.REACT_APP_FMS_API + '/transactions/deposits/logs';
const currentUser = getCurrentUser();

export const depositLogsAPI = createDynamicResourceAPI({
  resource: "DepositLogs",
  tagTypes: ["DepositLogs"],
      ...
      [`export${resource}`]: builder.mutation({
        ...
        async onQueryStarted(params, { queryFulfilled }) {
          try {
            ...
            link.href = url;
            link.download = `DepositLogs-${new Date().toISOString().split("T")[0]}.xlsx`;
...
export const {
  useGetDepositLogsQuery,
  useExportDepositLogsMutation
} = depositLogsAPI;
