...
const baseURL = process.env.REACT_APP_FMS_API + '/transactions/lddaps/logs';
const currentUser = getCurrentUser();

export const lddapLogsAPI = createDynamicResourceAPI({
  resource: "LDDAPLogs",
  tagTypes: ["LDDAPLogs"],
      ...
      [`export${resource}`]: builder.mutation({
        ...
        async onQueryStarted(params, { queryFulfilled }) {
          try {
            ...
            link.href = url;
            link.download = `LDDAPLogs-${new Date().toISOString().split("T")[0]}.xlsx`;
...
export const {
  useGetLDDAPLogsQuery,
  useExportLDDAPLogsMutation
} = lddapLogsAPI;
