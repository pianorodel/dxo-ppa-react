...
const baseURL = process.env.REACT_APP_FMS_API + '/transactions/noticeofcashallocations/logs';
const currentUser = getCurrentUser();

export const noticeofcashallocationLogsAPI = createDynamicResourceAPI({
  resource: "NoticeOfCashAllocationLogs",
  tagTypes: ["NoticeOfCashAllocationLogs"],
      ...
      [`export${resource}`]: builder.mutation({
        ...
        async onQueryStarted(params, { queryFulfilled }) {
          try {
            ...
            link.href = url;
            link.download = `NoticeOfCashAllocationLogs-${new Date().toISOString().split("T")[0]}.xlsx`;
...
export const {
  useGetNoticeOfCashAllocationLogsQuery,
  useExportNoticeOfCashAllocationLogsMutation
} = noticeofcashallocationLogsAPI;
