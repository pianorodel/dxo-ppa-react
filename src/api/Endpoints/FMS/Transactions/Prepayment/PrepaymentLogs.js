...
const baseURL = process.env.REACT_APP_FMS_API + '/transactions/prepayments/logs';
const currentUser = getCurrentUser();

export const prepaymentLogsAPI = createDynamicResourceAPI({
  resource: "PrepaymentLogs",
  tagTypes: ["PrepaymentLogs"],
      ...
      [`export${resource}`]: builder.mutation({
        ...
        async onQueryStarted(params, { queryFulfilled }) {
          try {
            ...
            link.href = url;
            link.download = `PrepaymentLogs-${new Date().toISOString().split("T")[0]}.xlsx`;
...
export const {
  useGetPrepaymentLogsQuery,
  useExportPrepaymentLogsMutation
} = prepaymentLogsAPI;
