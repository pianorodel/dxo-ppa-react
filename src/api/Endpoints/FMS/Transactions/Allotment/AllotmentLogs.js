...
const baseURL = process.env.REACT_APP_FMS_API + '/transactions/allotments/logs';
const currentUser = getCurrentUser();

export const allotmentLogsAPI = createDynamicResourceAPI({
  resource: "AllotmentLogs",
  tagTypes: ["AllotmentLogs"],
      ...
      [`export${resource}`]: builder.mutation({
        ...
        async onQueryStarted(params, { queryFulfilled }) {
          try {
            ...
            link.href = url;
            link.download = `AllotmentLogs-${new Date().toISOString().split("T")[0]}.xlsx`;
...
export const {
  useGetAllotmentLogsQuery,
  useExportAllotmentLogsMutation
} = allotmentLogsAPI;
