...
const baseURL = process.env.REACT_APP_FMS_API + '/transactions/appropriations/logs';
const currentUser = getCurrentUser();

export const appropriationLogsAPI = createDynamicResourceAPI({
  resource: "AppropriationLogs",
  tagTypes: ["AppropriationLogs"],
      ...
      [`export${resource}`]: builder.mutation({
        ...
        async onQueryStarted(params, { queryFulfilled }) {
          try {
            ...
            link.href = url;
            link.download = `AppropriationLogs-${new Date().toISOString().split("T")[0]}.xlsx`;
...
export const {
  useGetAppropriationLogsQuery,
  useExportAppropriationLogsMutation
} = appropriationLogsAPI;
