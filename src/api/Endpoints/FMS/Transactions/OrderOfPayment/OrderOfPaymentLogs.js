...
const baseURL = process.env.REACT_APP_FMS_API + '/transactions/orderofpayments/logs';
const currentUser = getCurrentUser();

export const orderofpaymentLogsAPI = createDynamicResourceAPI({
  resource: "OrderOfPaymentLogs",
  tagTypes: ["OrderOfPaymentLogs"],
      ...
      [`export${resource}`]: builder.mutation({
        ...
        async onQueryStarted(params, { queryFulfilled }) {
          try {
            ...
            link.href = url;
            link.download = `OrderOfPaymentLogs-${new Date().toISOString().split("T")[0]}.xlsx`;
...
export const {
  useGetOrderOfPaymentLogsQuery,
  useExportOrderOfPaymentLogsMutation
} = orderofpaymentLogsAPI;
