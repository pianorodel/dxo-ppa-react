...
export const getFundClustersColumns = (handleAction) => {
  const desktopColumns = [
    {
      ...
      cell: ({ row }) => {
        return <TextColumn onClick={() => handleAction("update", row.original)} text={row.original.fundClusterName} />;
      },
    },
    {
      header: "UACS",
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <StaticDropDownActions accessRights={FMS_ACCESS_RIGHTS.FMS_STATICDATA_FUNDCLUSTERS} handleActions={handleAction} data={data} />;
      },
    },
  ];
      ...
      cell: ({ row }) => {
        ...
        return (
          <CardBody onClick={() => handleAction("update", item)}>
            <LinkWithAvatarColumn name={item.fundClusterName} />
          </CardBody>
        );
      },
    },
