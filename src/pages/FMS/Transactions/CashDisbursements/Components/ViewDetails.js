...
import { useFindCashDisbursementsQuery } from "@/api/Endpoints/FMS/Transactions/CashDisbursement/CashDisbursements";
  ...
  return (
    ...
    <div style={{ paddingTop: 4 }}>
      <Section title="Request Details">
        <Row>
          <Col lg={6}>
            <InfoRow icon="bx bx-receipt" label="Payee" value={data?.Payee} />
          </Col>
          <Col lg={6}>
...
export default function ViewDetails({ show, onCloseClick, data, parentKey, onUpdateParentKey }) {
    ...
    () => [
      { key: "requestInfo", label: "Request Details", icon: "ri-user-line", component: TransactionDetailsTab },
      {
        key: "history", label: "History", icon: "ri-shield-flash-line", component: () =>
          <HistoryList
            transactionId={data?.cashDisbursementId}
  ...
  const { data: transactionData, refetch } = useFindCashDisbursementsQuery(
    { cashDisbursementId: data?.cashDisbursementId },
    { skip: !data?.cashDisbursementId, refetchOnMountOrArgChange: show },
  );
  ...
  return (
    <React.Fragment>
      <ActionModal
        ...
        cashDisbursementId={findData?.cashDisbursementId}
        data={state?.trxValue}
      ...
      <TransactionDetailsModal
        title={"Cash Disbursement Request"}
        show={show}
        data={state.data}
        tabs={TABS}
        buttons={BUTTONS}
