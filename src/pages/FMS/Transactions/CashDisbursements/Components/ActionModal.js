...
import { 
  ...
  useCancelCashDisbursementsMutation,
   useRejectCashDisbursementsMutation,
    useReturnCashDisbursementsMutation } from "@/api/Endpoints/FMS/Transactions/CashDisbursement/CashDisbursements";
...
const ActionModal = ({
  ...
  const {
    ...
    defaultValues: {
      cashDisbursementId: cashDisbursementId || 0,
      remarks: "",
    },
  });

  useEffect(() => {
    if (cashDisbursementId !== 0) {
      reset({
        cashDisbursementId: cashDisbursementId || 0,
        remarks: "",
      });
  ...
  }, [cashDisbursementId, show]);

  // Calling Action API
  const [returnAction, { isLoading: isReturning }] = useReturnCashDisbursementsMutation();
  const [approveAction, { isLoading: isApproving }] = useApproveCashDisbursementsMutation();
  const [rejectAction, { isLoading: isRejecting }] = useRejectCashDisbursementsMutation();
  const [cancelAction, { isLoading: isCanceling }] = useCancelCashDisbursementsMutation();
  ...
  const onSubmit = async (formData) => {
    const payload = {
      ...
      cashDisbursementId: cashDisbursementId,
      files
    };
