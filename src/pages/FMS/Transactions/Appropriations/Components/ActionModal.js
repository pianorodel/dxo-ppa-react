...
import { useApproveAppropriationsMutation, useCancelAppropriationsMutation, useRejectAppropriationsMutation, useReturnAppropriationsMutation } from "@/api/Endpoints/FMS/Transactions/Appropriation/Appropriations";
...
const ActionModal = ({
  ...
  const {
    ...
    defaultValues: {
      appropriationId: appropriationId || 0,
      remarks: "",
    },
  });

  useEffect(() => {
    if (appropriationId !== 0) {
      reset({
        appropriationId: appropriationId || 0,
        remarks: "",
      });
  ...
  }, [appropriationId, show]);

  // Calling Action API
  const [returnAction, { isLoading: isReturning }] = useReturnAppropriationsMutation();
  const [approveAction, { isLoading: isApproving }] = useApproveAppropriationsMutation();
  const [rejectAction, { isLoading: isRejecting }] = useRejectAppropriationsMutation();
  const [cancelAction, { isLoading: isCanceling }] = useCancelAppropriationsMutation();
