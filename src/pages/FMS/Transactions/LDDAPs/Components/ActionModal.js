...
import { useApproveLDDAPsMutation, useCancelLDDAPsMutation, useRejectLDDAPsMutation, useReturnLDDAPsMutation } from "@/api/Endpoints/FMS/Transactions/LDDAP/LDDAPs";
...
const ActionModal = ({
  ...
  // Calling Action API
  const [returnAction, { isLoading: isReturning }] = useReturnLDDAPsMutation();
  const [approveAction, { isLoading: isApproving }] = useApproveLDDAPsMutation();
  const [rejectAction, { isLoading: isRejecting }] = useRejectLDDAPsMutation();
  const [cancelAction, { isLoading: isCanceling }] = useCancelLDDAPsMutation();
