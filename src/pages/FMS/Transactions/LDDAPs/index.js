...
import { getLDDAPColumns } from "./Components/Columns";
...
const VIEWER_RIGHTS = [FMS_ACCESS_RIGHTS.FMS_TRANSACTIONS_LDDAPS_VIEWER];
const REQUESTOR_RIGHTS = [FMS_ACCESS_RIGHTS.FMS_TRANSACTIONS_LDDAPS_REQUESTOR];
const REQUESTOR_APPROVER_RIGHTS = [
  FMS_ACCESS_RIGHTS.FMS_TRANSACTIONS_LDDAPS_REQUESTOR,
  FMS_ACCESS_RIGHTS.FMS_TRANSACTIONS_LDDAPS_APPROVER,
];
...
const LDDAP = () => {
  ...
  const [deleteMutation] = useDeleteLDDAPsMutation();
  const [exportMutation] = useExportLDDAPsMutation();
  const [exportListForViewerMutation] = useExportListForViewerLDDAPsMutation();
  ...
  const resolvedQuery = useMemo(
    () => (isViewerOnly ? useGetListLDDAPsForViewersQuery : useGetLDDAPsQuery),
    [isViewerOnly],
  );
