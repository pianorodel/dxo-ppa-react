...
import { useApproveOrderOfPaymentsMutation, useCancelOrderOfPaymentsMutation, useRejectOrderOfPaymentsMutation, useReturnOrderOfPaymentsMutation } from "@/api/Endpoints/FMS/Transactions/OrderOfPayment/OrderOfPayments";
...
const ActionModal = ({
  ...
  const {
    ...
    defaultValues: {
      orderOfPaymentId: orderOfPaymentId || 0,
      remarks: "",
    },
  });

  useEffect(() => {
    if (orderOfPaymentId !== 0) {
      reset({
        orderOfPaymentId: orderOfPaymentId || 0,
        remarks: "",
      });
  ...
  }, [orderOfPaymentId, show]);

  // Calling Action API
  const [returnAction, { isLoading: isReturning }] = useReturnOrderOfPaymentsMutation();
  const [approveAction, { isLoading: isApproving }] = useApproveOrderOfPaymentsMutation();
  const [rejectAction, { isLoading: isRejecting }] = useRejectOrderOfPaymentsMutation();
  const [cancelAction, { isLoading: isCanceling }] = useCancelOrderOfPaymentsMutation();
