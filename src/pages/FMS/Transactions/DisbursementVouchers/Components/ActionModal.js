...
import { ApproveButton, CancelButton, CloseButton, RejectButton, ReturnButton } from "@/components/Common/Buttons";
...
const ActionModal = ({ disbursementVoucherId, data = null, show, onCloseClick, customFunction }) => {
  ...
  const {
    ...
    defaultValues: {
      disbursementVoucherId: disbursementVoucherId || 0,
      remarks: "",
    },
  });

  useEffect(() => {
    if (disbursementVoucherId !== 0) {
      reset({
        disbursementVoucherId: disbursementVoucherId || 0,
        remarks: "",
      });
  ...
  }, [disbursementVoucherId, show]);

  // Calling Action API
  const [returnAction, { isLoading: isReturning }] = useReturnDisbursementVouchersMutation();
  const [approveAction, { isLoading: isApproving }] = useApproveDisbursementVouchersMutation();
  const [rejectAction, { isLoading: isRejecting }] = useRejectDisbursementVouchersMutation();
  const [cancelAction, { isLoading: isCanceling }] = useCancelDisbursementVouchersMutation();
    ...
    try {
      if (actionType.current === "Approve") {
        ...
        if (approved?.data?.success) {
          ...
          notification({
            type: "success",
            header: `Approved ${MODULE_NAME}`,
            title: approved?.data?.returnData.referenceNo,
            message: "Request was Approved",
          });
          onCloseClick(true);
        } else {
          onCloseClick();
          notification({ type: "error", title: MODULE_NAME, message: `Error Approving - ${approved.data.returnMessage}` });
        }
        ...
        if (returned?.data?.success) {
          ...
          notification({
            type: "reject",
            header: `Returned ${MODULE_NAME}`,
            title: returned?.data?.returnData.referenceNo,
            message: "Request was Returned",
          });
          onCloseClick(true);
        } else {
          onCloseClick();
          notification({ type: "error", title: MODULE_NAME, message: `Error Returning - ${returned.data.returnMessage}` });
        }
        ...
        if (rejected?.data?.success) {
          ...
          notification({
            type: "reject",
            header: `Rejected ${MODULE_NAME}`,
            title: rejected?.data?.returnData.referenceNo,
            message: "Request was Rejected",
          });
          onCloseClick(true);
        } else {
          onCloseClick();
          notification({ type: "error", title: MODULE_NAME, message: `Error Rejecting - ${rejected.data.returnMessage}` });
        }
        ...
        if (canceled?.data?.success) {
          ...
          notification({
            type: "reject",
            header: `Canceled ${MODULE_NAME}`,
            title: canceled?.data?.returnData.referenceNo,
            message: "Request was Canceled",
          });
          onCloseClick(true);
        } else {
          onCloseClick();
          notification({ type: "error", title: MODULE_NAME, message: `Error Canceling - ${canceled.data.returnMessage}` });
        }
      }
    } catch (error) {
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="mb-3 pb-2">
              ...
              <Controller
                ...
                defaultValue=""
                render={({ field }) => (
                  <InputField control={control} type="textarea" rows="4" maxLength={500} showCharCounter placeholder="Enter Remarks" {...field} />
                )}
              />
              <br />
