...
const ActionModal = ({ checkDisbursementId, data = null, show, onCloseClick, customFunction }) => {
  ...
  const {
    ...
    defaultValues: {
      checkDisbursementId: checkDisbursementId || 0,
      remarks: "",
    },
  });

  useEffect(() => {
    if (checkDisbursementId !== 0) {
      reset({
        checkDisbursementId: checkDisbursementId || 0,
        remarks: "",
      });
  ...
  }, [checkDisbursementId, show]);

  // Calling Action API
  const [returnAction, { isLoading: isReturning }] = useReturnCheckDisbursementsMutation();
  const [approveAction, { isLoading: isApproving }] = useApproveCheckDisbursementsMutation();
  const [rejectAction, { isLoading: isRejecting }] = useRejectCheckDisbursementsMutation();
  const [cancelAction, { isLoading: isCanceling }] = useCancelCheckDisbursementsMutation();
  ...
  const onSubmit = async (formData) => {
    const payload = {
      ...
      checkDisbursementId: checkDisbursementId,
      files,
    };
