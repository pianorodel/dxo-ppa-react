...
import { useApproveNoticeOfCashAllocationsMutation, useCancelNoticeOfCashAllocationsMutation, useRejectNoticeOfCashAllocationsMutation, useReturnNoticeOfCashAllocationsMutation } from "@/api/Endpoints/FMS/Transactions/NoticeOfCashAllocation/NoticeOfCashAllocations";
...
const ActionModal = ({
  ...
  const {
    ...
    defaultValues: {
      noticeOfCashAllocationId: noticeOfCashAllocationId || 0,
      remarks: "",
    },
  });

  useEffect(() => {
    if (noticeOfCashAllocationId !== 0) {
      reset({
        noticeOfCashAllocationId: noticeOfCashAllocationId || 0,
        remarks: "",
      });
      ...
      reset({
        noticeOfCashAllocationId: 0,
        remarks: "",
      });
    }
  }, [noticeOfCashAllocationId, show]);

  // Calling Action API
  const [returnAction, { isLoading: isReturning }] = useReturnNoticeOfCashAllocationsMutation();
  const [approveAction, { isLoading: isApproving }] = useApproveNoticeOfCashAllocationsMutation();
  const [rejectAction, { isLoading: isRejecting }] = useRejectNoticeOfCashAllocationsMutation();
  const [cancelAction, { isLoading: isCanceling }] = useCancelNoticeOfCashAllocationsMutation();
  ...
  const onSubmit = async (formData) => {
    const payload = {
      ...
      noticeOfCashAllocationId: noticeOfCashAllocationId,
      files
    };
