...
import { useSaveBURsMutation } from "@/api/Endpoints/FMS/Budget/BURs";

const MODULE_NAME = "BUR";

const SaveModal = ({ data = null, show, onCloseClick, accessRights }) => {
  const [saveBUR, { isLoading: isSaving }] = useSaveBURsMutation();
  const { notification } = useNotificationModal();
  const isReadOnly = !hasWriteAccess(accessRights);
  const isUpdate = !!data?.burId;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await saveBUR(payload).unwrap();
