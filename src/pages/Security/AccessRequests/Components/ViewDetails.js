...
import { useFindAccessRequestsQuery } from "@/api/Endpoints/Core/Transactions/AccessRequest/AccessRequests";
    ...
    return (
        <div style={{ paddingTop: 4 }}>
            <Section title="Request Details">
                <Row>
                    <Col lg={6}>
                        <InfoRow icon="bx bx-user" label="Username" value={data?.userName} />
                    </Col>
                    <Col lg={6}>
                        <InfoRow icon="bx bx-id-card" label="Full Name" value={data?.fullName} />
                    </Col>
                    <Col lg={6}>
                        <InfoRow icon="bx bx-envelope" label="Email Address" value={data?.emailAddress} />
                    </Col>
                    <Col lg={6}>
                        <InfoRow icon="bx bx-phone" label="Mobile No." value={data?.mobileNo} />
                    </Col>
                </Row>
            </Section>
        ...
        () => [
            { key: "requestInfo", label: "Request Details", icon: "ri-user-line", component: TransactionDetailsTab },
            {
                key: "history", label: "History", icon: "ri-shield-flash-line", component: () =>
                    <HistoryList
                        transactionId={data?.accessRequestId}
            ...
            //             transactionId={data?.accessRequestId}
            //             useFilesQuery={useGetFilesPrescriptionRequestsQuery}
            //             idKey="accessRequestId"
            //             title="Files"
            //         />
            // },
    ...
    return (
        <React.Fragment>
            <ActionModal
                ...
                accessRequestId={findData?.accessRequestId}
                data={state?.trxValue}
