...
const ActionModal = ({ accessRequestId, data = null, show, onCloseClick, customFunction }) => {
  ...
  const {
    ...
    defaultValues: {
      accessRequestId: accessRequestId || 0,
      remarks: "",
    },
  });
  ...
  }, [accessRequestId, show]);

  // Calling Action API
  const [returnAction, { isLoading: isReturning }] = useReturnAccessRequestsMutation();
  const [approveAction, { isLoading: isApproving }] = useApproveAccessRequestsMutation();
  const [rejectAction, { isLoading: isRejecting }] = useRejectAccessRequestsMutation();
  const [cancelAction, { isLoading: isCanceling }] = useCancelAccessRequestsMutation();
  ...
  const onSubmit = async (formData) => {
    const roleIds = (formData.roles || []).map((r) => ({ roleId: r.value || r.roleId, roleName: r.label || r.roleName }));
    const payload = {
