...
const VerticalLayout = (props) => {
    ...
    useEffect(() => {
        ...
        const initMenu = () => {
            ...
            let matchingMenuItem = itemsArray
                .filter(x => pathName.startsWith(x.pathname))
            ...
            {(navData || []).map((item, key) => {
                return (
                    <React.Fragment key={key}>
                        {item['isHeader'] ?
                            <li className="menu-title"><span data-key="t-menu" style={{ color: item.color }}>{item.label}</span></li>
                            : (
                                (item.subItems ? (
