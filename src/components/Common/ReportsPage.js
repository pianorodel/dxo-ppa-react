...
const ReportsPage = ({ title = "Reports", reports = [], breadCrumbs = [] }) => {
    ...
    }, [reports, search, activeGroup]);

    const totalAll = reports.reduce((s, g) => s + g.items.filter(i => hasAccess(i.permissionTypeId)).length, 0);

    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title={"Reports"} crumbs={breadCrumbs || []} />
                    {/* ── Hero banner ─────────────────────────────────────────── */}
                                ...
                                <Col md={7}>
                                    <div className="d-flex align-items-center gap-3">
                                        <div className="rounded-3 d-flex align-items-center justify-content-center flex-shrink-0"
                                            ...
                                            <i className="ri-file-chart-line text-white" style={{ fontSize: "1.4rem" }} />
                                        </div>
                                        <div>
                                            ...
                                            <p className="mb-0" style={{ color: "rgba(255,255,255,0.75)", fontSize: "0.85rem" }}>
                                                {totalAll} report template{totalAll !== 1 ? "s" : ""} · Select and generate any report below.
                                            </p>
                                        </div>
                                    </div>
                                </Col>
                    ...
                    {filtered.length === 0 ? (
                        <Card className="border-0 shadow-sm">
                            <CardBody className="text-center py-5">
                                <i className="ri-file-chart-line d-block mb-3 opacity-25" style={{ fontSize: "3rem" }} />
                                                    ...
                                                    <CardBody className="p-4 d-flex flex-column">
                                                        ...
                                                        <div className="mt-3 pt-3" style={{ borderTop: "1px solid var(--vz-border-color)" }}>
                                                            <Link
                                                                ...
                                                                <i className="ri-file-chart-line me-1" />Configure
                                                            </Link>
                                                        </div>
                                                    </CardBody>
                                                </Card>
